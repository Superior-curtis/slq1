...
import { registerPornhubProxy } from "../pornhubProxy";
...
async function startServer() {
  ...
  // Protect against malformed encoded URI components that crash Express router
  app.use((req, res, next) => {
    try {
      // attempt to decode to trigger any decode URI errors early
      decodeURIComponent(req.path);
      next();
    } catch (err) {
      console.warn("[Server] Malformed URI path:", req.path);
